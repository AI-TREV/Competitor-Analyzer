...
**8 tables total.** Forensic review added `internal_price_history`, new constraints, and staleness tracking fields. See `.paul/INTEGRATION.md` for full rationale per change.
...
```sql
-- last_scraped_at + last_scrape_status: dashboard store freshness display
...
```sql
-- Dated price history — Phase 5 joins here (not current_price) for accurate historical gap analysis
CREATE TABLE internal_price_history (
...
```sql
-- UNIQUE constraint prevents duplicate product rows across scrape runs
CREATE TABLE competitor_product (
  ...
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE(competitor_id, competitor_name, pack_size)
);
```
...
```sql
-- observed_at: NO DEFAULT — scraper must supply HTTP response time
-- screenshot_path: RELATIVE only ("{run_id}/{filename}.png") — CHECK enforces this
-- UNIQUE: prevents duplicate observations on re-run
...
```sql
-- scrape_run_id MUST be crypto.randomUUID() — never sequential integer or timestamp
...
```sql
-- exception_type values: MATCH_CONFIDENCE_LOW | PARSE_FAILED | PRICE_ANOMALY | JSON_INVALID | SITE_BLOCKED | SKU_REF_ERROR
